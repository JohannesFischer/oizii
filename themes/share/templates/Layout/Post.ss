...
<section class="row post" ng-swipe-left="nextPost()" ng-hide="loading">
  <section class="columns large-9 medium-9">
    ...
    <p ng-mouseover="getLikes()" title="{{ likeUsers }}" ng-hide="post.Likes < 1">
      {{ post.Likes }} like<span ng-hide="post.Likes == 1">s</span>
    </p>

    <a href="<% if not CurrentMember %>/#/login<% end_if %>" class="button like"<% if CurrentMember %> ng-click="sendLike()"<% end_if %>>{{post.HasLiked ? "Unlike" : "Like"}}</a>
